    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-8">
        <div className="card max-w-2xl w-full text-center">
          <h2 className="text-3xl font-bold mb-4">No Quiz Results</h2>
          <p className="text-gray-600 mb-6">Take a quiz to see your results!</p>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {newAchievements.length > 0 && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 animate-fade-in">
            <div className="bg-white rounded-xl p-8 max-w-md w-full animate-bounce-subtle">
              <h3 className="text-2xl font-bold text-center mb-6">🎉 New Achievements!</h3>
              <div className="space-y-4">
                {newAchievements.map((achievement, index) => (
                  <div key={index} className="flex items-center gap-4 p-4 bg-yellow-50 rounded-lg border-2 border-yellow-300">
        ...
        {showReview && incorrectAnswers.length > 0 && (
          <div className="max-w-3xl mx-auto animate-slide-up">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold text-gray-800">Review Your Mistakes</h2>
              <button
            ...
            <div className="space-y-4">
              {incorrectAnswers.map((answer, index) => (
                <div key={index} className="card">
                  <h3 className="font-bold text-lg mb-3">{answer.question}</h3>
                  <div className="space-y-2 mb-4">
                    <div className="p-3 bg-red-50 border-2 border-red-300 rounded-lg">
                      <span className="text-sm font-semibold text-red-700">Your Answer:</span>
                      <p className="text-gray-800">{answer.options[answer.selectedAnswer]}</p>
                    </div>
                    <div className="p-3 bg-green-50 border-2 border-green-300 rounded-lg">
                      <span className="text-sm font-semibold text-green-700">Correct Answer:</span>
                      <p className="text-gray-800">{answer.options[answer.correctAnswer]}</p>
                    </div>
                  </div>
